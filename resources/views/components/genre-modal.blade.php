<div x-data="{show: false, tooltip: false}"  @keydown.escape="window.genreModal().reboot(); show=false;" class="relative my-4" x-init="window.genreModal().getCheckedLabelsText()">
    ...
    <div x-show="show" class="z-50 relative">
        <div class="fixed bg-gray-800 bg-opacity-95 top-0 left-0 right-0 bottom-0 flex justify-center items-center">
            <div class="bg-white p-8 max-w-5xl w-9/12" @click.away.prevent="show=false">
                <button @click.prevent="window.genreModal().reboot(); show= false"class="float-right text-gray-50 w-6 h-6 bg-red-700 rounded-full flex justify-center items-center">&times;</button>
                ...
                <button type="button" @click="window.genreModal().save(); show= false" class="bg-blue-700 text-gray-100 px-3 py-2 mt-2">sauvegarder</button>
                <button @click.prevent="window.genreModal().reboot(); show= false"class="bg-red-700 text-gray-100 px-3 py-2 mt-2">Fermer</button>
            </div>
        </div>
    </div>
