<div class="flex m-2 sm:m-4 comment" style="word-break: break-word">
    ...
    <div class="ml-4 text-gray-50 w-full">
        <div class="mb-2">
            <h1 class="comment__author {{ $item->author->is_logged_in_user() ? 'text-red-400' : 'text-blue-500' }}"> {{ $this->isDifferent() ? $item->author->name : 'Moi' }} </h1>
            <span class="comment__date"> {{ Carbon\Carbon::parse($item->created_at)->diffForHumans() }} </span>
            @unless($this->isDifferent())<button wire:click="destroy" class="float-right text-red-500 text-3xl">&times;</button>@endunless
        </div>
